...
namespace Events
{
	void MenuOpenCloseEventSink::Install(const std::set<std::string>* a_successTopics, const std::set<std::string>* a_failureTopics) noexcept
	{
	...
	RE::BSEventNotifyControl MenuOpenCloseEventSink::ProcessEvent(const RE::MenuOpenCloseEvent* a_event, RE::BSTEventSource<RE::MenuOpenCloseEvent>*)
	{
		if (a_event->menuName == RE::DialogueMenu::MENU_NAME && a_event->opening) {
			Scaleform::SetEntryTextFunctionHandler::Install(successTopics, failureTopics);
		}
